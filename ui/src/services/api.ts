...
export const healthCardService = {
  ...
  getByPatient: (patientId: string) =>
    api.get<HealthCard[]>(`/health-cards/patient/${patientId}`),
  create: (data: Omit<HealthCard, "id" | "createdAt" | "updatedAt">) =>
    api.post<HealthCard>("/health-cards", data),
  update: (id: string, data: Partial<HealthCard>) =>
    api.patch<HealthCard>(`/health-cards/${id}`, data),
...
export const authService = {
  ...
  register: (data: {
    ...
    password: string;
    role?: string;
  }) => api.post("/auth/register", { ...data, role: "paciente" }),
